class Circle {
    constructor(x, y, color, distance, radius, sx = 0, sy = 0) {
        ...
        this.ball = function () {
            c.beginPath();
            c.arc(this.newx, this.newy, this.radius, 0, Math.PI * 2);
        ...
        this.moveChaos = function () {
            this.x = tx;
            this.y = ty;
            this.newx = -this.x * this.x + t * t + this.x * this.y + this.x * t - this.x - this.y - t;
            this.newy = -this.y - this.x * t - this.x * this.y;
            // this.newy = this.y * 2;
            // this.newx = Math.sqrt(1 - this.y * this.y);
            // this.newy = Math.sqrt(1 - this.x * this.x);
            // this.newx = -this.y * this.y - this.x * t + this.y;
            // this.newy = this.x * this.x - this.x * this.y + t;
            tx = this.newx;
            ty = this.newy;
        }
